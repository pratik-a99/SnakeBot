...
class RotationAlongAxis(Gait):
    def __init__(self, publishers, num_joints=10):
        ...
        self.amplitudes, self.omegas, self.phases, self.axis_lags = self.sinewave_params()
        self.publishers = publishers

    def sinewave_params(self):
...
def get_publisher_nodes(num_joints):
    ...
    for i in range(num_joints):
        pub_node_name = '/snakebot_v1/joint_{}_position_controller/command'.format(i+1)
...
if __name__=="__main__":
    ...
    rospy.init_node('snakebot_rotation_along_axis', anonymous=True)
    
    num_joints = 10
    joint_publishers = get_publisher_nodes(num_joints=num_joints)
    gait = RotationAlongAxis(publishers=joint_publishers, num_joints=num_joints)
