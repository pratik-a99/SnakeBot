...
def get_publisher_nodes(num_joints):
    ...
    for i in range(num_joints):
        pub_node_name = '/snakebot_v1/joint_{}_position_controller/command'.format(i)
...
if __name__=="__main__":
    ...
    rospy.init_node('snakebot_corkscrew', anonymous=True)
    
    num_joints = 10
    joint_publishers = get_publisher_nodes(num_joints=num_joints)
    gait = CorkScrewGait(publishers=joint_publishers, num_joints=num_joints)
