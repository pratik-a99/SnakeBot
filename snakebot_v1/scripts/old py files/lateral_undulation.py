...
class LateralUndulationGait(Gait):
    ...
    def get_angle_params(self, num_joints):
        amp_reduction_factor_hor = 1.7 #1.5 #0.4
        amp_reduction_factor_ver = 1.7 #1.5 #0.1
...
if __name__=="__main__":
    ...
    rospy.init_node('snakebot_lateral_undulation', anonymous=True)
    
    num_joints = 10
    joint_publishers = get_publisher_nodes(num_joints=num_joints)
    gait = LateralUndulationGait(publishers=joint_publishers, num_joints=num_joints)
